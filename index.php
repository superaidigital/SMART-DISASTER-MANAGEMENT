 ...
 * FIX: ย้ายการตรวจสอบ Session ขึ้นมาบนสุดเพื่อป้องกัน error "Headers already sent"
 */

// 1. เริ่ม Session และโหลดค่าคอนฟิกพื้นฐานก่อนเริ่มส่ง HTML ใดๆ
session_start();
require_once 'config/db.php';
require_once 'includes/functions.php';

// 2. ตรวจสอบการเข้าสู่ระบบ (ต้องทำก่อน require_once 'includes/header.php')
if (!isset($_SESSION['user_id'])) {
    header("Location: login.php");
    exit();
}

// 3. เตรียมข้อมูลสำหรับ Dashboard
...
try {
    if ($role === 'admin') {
        ...
        $stats['total_evacuees'] = $data['current_stay'] ?? 0;
        $stats['vulnerable'] = $data['vulnerable'] ?? 0;
        $stats['pending_requests'] = $data['pending_req'] ?? 0;
        $stats['low_stock'] = $data['low_inv'] ?? 0;
...
// 4. โหลด Header (เริ่มส่ง HTML ไปยัง Browser ที่นี่)
...
<div class="container-fluid py-4">
    ...
    <div class="row mb-4">
        <div class="col-md-7">
            <h2 class="fw-bold text-dark mb-1">สวัสดีคุณ <?php echo htmlspecialchars($_SESSION['username']); ?></h2>
